...
gemeinden_landesgrenze <- gemeinden_landesgrenze %>%
  mutate(see_grenze = bfs_nummer %in% see_grenzen_gemeinden) %>% 
  mutate(erreichbar = !bfs_nummer %in% erreichbar)
...
plz_erreichbar <-  check_spatial_relationship(plz_landesgrenze_line, roads_union, st_intersects, "erreichbar")
# filter only erreichbar=FALSE for the join because of the Poly to Line conversion (that results in more rows, which complicates the join)

plz_nicht_erreichbar <- plz_erreichbar %>% 
  filter(erreichbar == FALSE | ZIP4 == 3920)# manually add Zermatt (has road access but its restricted)

plz_nicht_erreichbar_toJoin <-plz_nicht_erreichbar %>% st_drop_geometry() %>% select(erreichbar, ZIP4)

plz_landesgrenze_join <- plz_landesgrenze %>% left_join(plz_nicht_erreichbar_toJoin, by=c("ZIP4"))
# set NA values to TRUE as only FALSE values where joined before
plz_landesgrenze_join <- plz_landesgrenze_join %>%
  mutate(erreichbar = is.na(erreichbar))
...
plz_bfs <- attach_intersecting_values(plz_landesgrenze_join, gemeinden_landesgrenze %>% st_buffer(-20), "bfs_nummer", "bfs_nummer")

# clear attributes
plz_bfs <- plz_bfs %>% select(-c("id"))
gemeinden_landesgrenze <- gemeinden_landesgrenze %>% select(bfs_nummer, name, grenz_gemeinde, see_grenze, erreichbar)
...
gemeinden_join <- gemeinden_join %>% filter(erreichbar == TRUE)
plz_join <- plz_join %>% filter(erreichbar == TRUE)

gemeinden_join_wgs_84 <- gemeinden_join %>% st_transform(4326)
...
```{r}
compare_column_values(plz, plz_centers, "ZIP4")
```
